...
import { ChevronDown, ChevronRight, School, Calendar, MapPin, TrendingUp } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import { useToast } from "@/hooks/use-toast";
import type { AcademicSpark } from "./types";

interface AcademicSparkListProps {
  sparks: AcademicSpark[];
  setSparks: React.Dispatch<React.SetStateAction<AcademicSpark[]>>;
  totalSparkCount: number;
}

export default function AcademicSparkList({ sparks, setSparks, totalSparkCount }: AcademicSparkListProps) {
    ...
    return (
      <Card>
        <CardContent className="p-8 text-center">
          ...
          <h3 className="text-lg font-semibold mb-2">暂无学术火种</h3>
          <p className="text-muted-foreground">
            还没有学术火种数据。火种包含学校-专业的详细升学就业信息。
          </p>
        </CardContent>
      </Card>
    );
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium text-muted-foreground">
          显示 {sparks.length} / {totalSparkCount} 个学术火种
        </h3>
      </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <CardTitle className="text-base">
                        {spark.school_name} - {spark.major_name}
                      </CardTitle>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      {new Date(spark.updated_at).toLocaleString('zh-CN')}
                    </div>
                  </div>

                  {spark.academic_spark_attributes && spark.academic_spark_attributes.length > 0 && (
                    <div>
                      <h4 className="font-medium mb-2">火种属性：</h4>
                      <div className="grid gap-2">
                        {spark.academic_spark_attributes.map((attr) => (
